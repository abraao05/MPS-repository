...
| Definição colaborativa da arquitetura | A arquitetura em 3 camadas foi definida na Fase 1 com participação de toda a equipe técnica (Cezar Hiraki, Henry), documentada nos contratos de interface do Core como referência compartilhada para o desenvolvimento subsequente. |
| Pareamento técnico (pair programming pontual) | Sessões pontuais entre Henry (Desenvolvedor) e Allan Alves (Desenvolvedor) para implementação das partes de maior complexidade técnica: `BuildAdfDocument` (geração de ADF) e `HtmlHelper` (conversão de HTML). |
