...
> Imagens em `images/` (evidenciam o Azure DevOps). Gerado em 15/06/2026.
...
| RASTR | REQ/GRE | Jira AG↔PR; PR referenciando história | `pull_requests/` (PR) | ⚠️ falta Jira |
...
| VV | VV | **Pipeline** como gate; **branch policy** (PR+CI) | `branches/` (parcial) | ❌ falta pipeline+policy |
...
| 4 | **Pipeline CI/CD** (build verde, 22 testes, cobertura) | GCO, MED, VV, REL-VV | furo | criar `azure-pipelines.yml`+testes (build real no Azure) OU abrandar docs |
| 5 | **Branch policy** (PR+CI obrigatório no develop) | VV, GCO | furo | configurar policy no Azure + printar OU abrandar docs |
