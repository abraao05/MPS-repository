...
|---|---|---|---|
| A-01 | Pipeline de CI executado em agente Linux padrão | Pipelines executadas no runner **runner-vm-docker** (GitLab CI) nas três pipelines ("MilhasFacil API/Web/Crawler - Pipeline") | Runner Docker é o recurso disponível na organização; padronização das pipelines GitLab CI reduz divergência entre os três serviços. Mitiga o risco R-05 (pipeline de CI) |
| A-02 | Histórico de repositório construído incrementalmente desde a primeira sprint | Histórico dos repositórios (MilhasFacil_api, MilhasFacil_web, MilhasFacil_crawler) **inicializado de forma consolidada** (retroativa); saneamento prospectivo a partir da S9 | A integração ao GitLab ocorreu de forma retroativa; datas reais de MR e pipeline concentram-se em 13–15/06/2026. Todos os **39 MRs** estão com **2 revisores aprovados** (verificado via SQL). A partir da S9 a revisão de MR está **evidenciada** nos registros do GitLab; foi ativada a **branch protection rule** de revisor (≥1 revisor) em develop nos três repositórios, impedindo recorrência |
...
| A-04 | Revisão de MR com dois revisores independentes | Revisão com **2 revisores aprovados** em todos os 39 MRs — verificado via SQL; os revisores reais incluem cezar.velazquez (Tech Lead) e lucas.batista, entre outros | Equipe enxuta com três desenvolvedores efetivos; o Tech Lead lidera a aprovação técnica dos MRs, mantendo o gate de aprovação sem ampliar o lead time. A branch protection rule de revisor em develop (S9) torna o revisor obrigatório nos três repositórios. A aprovação de escopo/CR é responsabilidade do GP (Abraão) |
...
| A-09 | Nomenclatura de scripts SQL `0001-create-…-table.sql` (GUIA-GCO-001 §4) | Migrations versionadas pela convenção do **Flyway** (`V1__create_users.sql` … `V5__…`; `V9__airport_search_index.sql`); padronização de nomenclatura de BD em curso via **api !15 (MF-73)** com `V10__fix_naming_conventions.sql` | A ferramenta Flyway exige o prefixo `V<n>__`; a numeração sequencial e o controle de versão do schema (objetivo do guia) são plenamente atendidos pela convenção nativa. O MR api !15 (MF-73, chore — padronização de nomenclatura de BD conforme GUIA-GCO-001) introduz a migration V10 (padronização de índices + coluna `is_active`) e foi **aprovado por cezar.velazquez + lucas.batista no GitLab**, aguardando merge |
...
- Controle de versão de código no GitLab (Git), com branch padrão main nos três repositórios.
- MR obrigatório para develop antes do merge, com aprovação do Tech Lead (Cézar Velazquez).
...
- Rastreabilidade de requisitos (RF → Jira → branch → MR → build).
...
|---|---|
| A-01 (CI GitLab / runner-vm-docker) | Padronização das três pipelines; pipelines GitLab CI executadas com sucesso. Risco R-05 mantido sob controle |
| A-02 (histórico consolidado) | Todos os 39 MRs com 2 revisores aprovados (verificado via SQL). Meta "MRs sem revisor = 0" — Cumprida |
...
| A-08 / A-09 (saneamento de BD e branch) | Branch protection rule de revisor ativada em develop (saneamento da política de branch); padronização de nomenclatura de BD em curso via api !15 (MF-73), migration V10 aprovada por cezar.velazquez + lucas.batista, aguardando merge |
...
| 1.2 | 26/06/2026 | Time de Melhoria Contínua | Atualização da plataforma de Azure DevOps para GitLab (Git); runner-vm-docker no lugar de PowerShell@2/agente Windows; revisores reais confirmados (cezar.velazquez + lucas.batista); 37 MRs com 2 revisores aprovados via SQL; referências a PR #29 → api !15; "Mateus Veloso" removido como alias. |
| 1.3 | 29/06/2026 | Auditoria MPS.BR Nível C | Contagem de MRs atualizada 37 → 39 (inclusão de api !20/!21 — correção de build develop/homolog) em §3 (A-02/A-04) e §5; terminologia "PR" → "MR" em §3 (A-06), §4 e §5. |
