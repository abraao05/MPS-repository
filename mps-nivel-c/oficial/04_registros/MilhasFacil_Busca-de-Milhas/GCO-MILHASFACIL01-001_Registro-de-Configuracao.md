...
> **Nota de equivalência:** os nomes de revisores e assignees observados na API do Azure DevOps/Jira correspondem, no time atual, ao Tech Lead Cézar Velazquez (a aprovação de PR aparece sob a conta legada `Mateus Veloso` e os commits de infra/arquitetura sob `Raony Chagas`/`Mateus Sousa`, todos = Cézar). Em texto de gestão são usados os nomes reais do time.
