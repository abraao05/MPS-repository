...
- **MR obrigatório** para o branch `develop`; não há merge direto.
...
Levantamento da API do GitLab em 29/06/2026: **39 Merge Requests** distribuídos pelos três repositórios — **38 concluídos** e **1 ativo** (api !15, MF-73, aprovado por cezar.velazquez + lucas.batista, aguardando merge). Todos os 39 MRs possuem exatamente 2 revisores aprovados (verificado via SQL).

| Repositório | MRs S1–S8 (concluídos) | MRs S9 — funcionais (concluídos) | MRs release/docs/CI S9-S10 (concluídos) | MR ativo |
|---|---|---|---|---|
| MilhasFacil_api | !1–!11 (11 MRs) | !12, !13, !14 | !16 (docs), !17 (release), !18, !19 (CI config), !20, !21 (build fix) | !15 (MF-73) |
...
Todos os 39 MRs possuem **exatamente 2 revisores aprovados** registrados em `merge_request_reviewers` (verificado em 26/06/2026 — 0 linhas com contagem ≠ 2).
...
| api !16 (docs), api !17 (release), api !18–!21, web !11/!12, crawler !5/!6 (10 MRs) | S9–S10 | Concluído | 2 revisores (lucas.batista / felipe.siqueira / cezar.velazquez + abraao.oliveira) | Aprovados |
| api !15 (MF-73) | S10 | **Ativo (aprovado, aguardando merge)** | cezar.velazquez + lucas.batista | Aprovados |

**Observações:**

- Todos os **39 MRs** possuem 2 revisores aprovados registrados. A meta "MRs sem revisor = 0" está **plenamente cumprida**.
...
| 10 MRs de release/docs/CI e correção de build S9-S10 concluídos (incl. api !20/!21), 2 revisores aprovados por MR | 29/06/2026 | lucas.batista / felipe.siqueira / cezar.velazquez + abraao.oliveira |
| **Meta: todos os 39 MRs com 2 revisores = Cumprida** | 29/06/2026 | Time de Melhoria Contínua |
...
| 4.0 | 29/06/2026 | Auditoria MPS.BR Nível C | Contagem 37 → 39 MRs (inclusão de api !20/!21 — correção de build develop/homolog) em §1 (política), §3 e §4; terminologia "PR obrigatório" → "MR obrigatório" em §1. |
