# ATA DE ACOMPANHAMENTO — Fase 4 — Tratamento de erros e validação

ATA-AASPAP01-005 · Versão 1.0 · 12/06/2026 · Timeware Brasil

| Campo | Valor |
|---|---|
| **Documento** | ATA-AASPAP01-005 |
| **Projeto** | AASP_AndamentosProcessuais — Refatoração da Solução de Captura de Andamentos |
| **Cliente** | AASP — Associação dos Advogados de São Paulo |
| **Organização** | Timeware Brasil Softwares e Serviços LTDA |
| **Gerente de Projeto** | Abraão Oliveira |
| **Versão** | 1.0 |
| **Data** | 12/06/2026 |
| **Tipo** | Reunião de acompanhamento de projeto (cadência por fase/marco) |
| **Local / meio** | Videoconferência |
| **Responsável pela ata** | Abraão Oliveira |

## 1. Participantes
| Nome | Papel |
|---|---|
| Abraão Oliveira | Gerente de Projeto |
| Cézar Hiraki Velázquez | Tech Lead / DevOps / Arquiteto |
| Raony Chagas | Desenvolvedor Sênior |
| Mateus Veloso | Desenvolvedor (Suporte) |
| Caroline Sousa | Analista de QA |
| Marcos Correa Fernandez Turnes | Sponsor / PO (AASP) |

## 2. Pauta
- Tratamento de erro e segredo por instância (CR-AASPAP01-002)
- Resultado dos testes (CT-01 a CT-12)
- Atraso de ~30 dias acordado

## 3. Discussões e definições
Concluída a Fase 4: campos Observacao e Segredo por instância, tratamento granular dos retornos das parceiras e priorização do RunUpdater (CR-AASPAP01-002). Os 12 cenários de teste (CT-01 a CT-12) foram 100% aprovados; BUG-02 a BUG-05 corrigidos antes da implantação (contenção 100%). O atraso de ~30 dias foi acordado com o cliente e absorvido, mantido o prazo final.

## 4. Decisões e aceites
| Decisão / aceite | Responsável | Data |
|---|---|---|
| Aceite da Fase 4 (validação e tratamento de erros) | Marcos Correa Fernandez Turnes | 12/06/2026 |
| CR-AASPAP01-002 aprovado | Marcos Correa Fernandez Turnes | Mai/2026 |

## 5. Ações (follow-up)
| Ação | Responsável | Prazo |
|---|---|---|
| Montar pacote de GMUD e iniciar a implantação em produção (Fase 5) | Cézar Hiraki Velázquez / Lucas Batista | Jun/2026 |

## 6. Próximos passos
Fase 5 — Implantação (em andamento; encerramento previsto 31/08/2026).

## Histórico de revisões
| Versão | Data | Autor | Descrição |
|---|---|---|---|
| 1.0 | 12/06/2026 | Gerente de Projeto | Ata de acompanhamento da Fase 4, consolidada a partir de MED-AASPAP01-001, CR-AASPAP01-002 e REL-VV-AASPAP01-001. |
