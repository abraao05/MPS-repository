...
| 3 | 2026-06-15 | **Criar planilha do Painel de Portfólio (REG-OSW-001)** | O REG-OSW-001 hoje é só `.md`/`.docx` em prosa, mas é um "painel" — dado tabular vivo. O próprio §2/§5.1 diz que a situação dos projetos e o quadro de capacity são mantidos em Jira/Confluence. Criar uma planilha anexa (instrumento operacional) com abas: **Portfólio** (projeto, cliente, natureza, período, prioridade, status), **Quadro de Capacity** (colaborador, projeto, alocação %, período, status) e **Acompanhamento** (projeto, compromissos, desvios, tratamento — OSW 10). O `.docx` narrativo passa a referenciar a planilha. Usar só o conteúdo já documentado nos 5 projetos; o **% de alocação por colaborador** no capacity ainda não existe — deixar coluna estruturada e em branco, sem inventar. | ⬜ A fazer (adiado pelo ponto focal — "depois fazemos isso") |
